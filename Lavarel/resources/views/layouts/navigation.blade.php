...
<nav x-data="{ open: false }" class="bg-white border-b border-gray-100 md:ml-60">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    <x-nav-link x-bind:href="route('dashboard')" :active="{{ request()->routeIs('dashboard') ? 'true' : 'false' }}">
                        {{ __('Dashboard') }}
                    </x-nav-link>
                </div>
            </div>

            <!-- No user/account links: navigation is fully public -->
    ...
    <div x-bind:class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link x-bind:href="route('dashboard')" :active="{{ request()->routeIs('dashboard') ? 'true' : 'false' }}">
                {{ __('Dashboard') }}
